...
def names():
    ...
    if request.method == 'POST':
        ...
        if sign == 'on' and split == 'on':
            ...
            try:
                ...
                response = make_response(
                    send_file(zip_filename, as_attachment=True))
                ...
                status_updates[username] = {
                    "status": f"Signing , Splitting is done and total i20's are {result}  "}
                # print(msg)
                ...
                status_updates[username] = {
                    "status": f"Signing and Splitting failed with error {e}  "}
                issm_log.logger.error(
                    f"Failed in sign=='on' and split== 'on',error{e}")
        # If only signature is selected  then signature is added and zip file is sent as attachment
            ...
            status_updates[username] = {
                "status": f"Add signature selected ..Processing..."}

            try:
                issm_log.logger.info("Received for Sign ")
                issm_log.logger.info(
                    f"Received file {pdf_filename} , DSO  {dso}")
                # signature is added
                depensignature(pdf_filename, signature_file,
                               output_file, length, width, xco, yco)
                # add_signature1(pdf_filename, signature_file, output_file, length, width, xco, yco)
                # print(output_file)
                response = make_response(
                ...
                status_updates[username] = {
                    "status": f"Adding signature failed with error {e}"}
                issm_log.logger.error(
                    f"Error in sign=='on' and split==None,error {e}")
        # if only split is selected
            ...
            try:
                ...
                issm_log.logger.error(
                    f"Error in Splitting , route /upload3 {e}")
        else:
            status_updates[username] = {
                "status": "Nothing selected in DSO Signature"}
...
def upload():
    if request.method == 'POST':
        try:
            ...
            issm_log.logger.info(
                f"Intial I20. Received post request with files :{pdf_filename,issm,slate}")
            # Total number of pages in i20
            num_pages = pages(pdf_filename)
            # sevid = ""
            # socketio.emit(
            #     'update', {'status': 'Starting upload process...'}, room=session_id)
            ...
            try:
                ...
                sevid, totalpages, totalsigns = splitsignature(
                    pdf_filename, signature_file, length, width, xcoordinate, ycoordinate)
                ...
                status_updates[user][
                    "status"] = f"Failed during adding signature and splitting {e} "
            ...
            try:
                ...
                if slaterequest == 'y':
                    ...
                    if stream == 'g':
                        ...
                        issm_log.logger.info(
                            f"Response of files to slate {output}, stream is {stream}")
    ...
    if request.method == 'GET':
        ...
        result = [s for s in [splitFailure, indexError,
                              zipMessage] if s is not None and s != ""]
...
def login():
    if request.method == 'POST':
        ...
        try:
            ...
            encoded_credentials = request.headers.get(
                'Authorization').split(' ')[-1]
            decoded_credentials = base64.b64decode(
                encoded_credentials).decode().split(':')
              ...
              #  print("/*/*/*/",result)
                if login_result == http.HTTPStatus.OK:
                 #   print("in logn ")
                    ...
                    token_exp = datetime.datetime.utcnow() + timedelta(hours=1)
                    token_payload = {'username': username,
                                     'role': role, 'exp': token_exp}
                    # Encode token with secret key
                    token = jwt.encode(
                        token_payload, 'secret', algorithm='HS256')
                 #   print("78",token)
                    response = make_response({'message': 'Login successful', 'data': {
                                             'role': role, 'username': username, 'fullname': fullname, 'institutionname': institutionname}})
                ...
                if result == http.HTTPStatus.UNAUTHORIZED:
                    issm_log.logger.info(
                        f"Invalid username or password entered by {username}")

                    response = make_response(
                        {'message': 'Unauthorized user'}, http.HTTPStatus.UNAUTHORIZED)
                    return response
                else:
...
def register():
    if request.method == 'POST':
        ...
        register_result = registeruser(
            username, password, email, role, fullname, institutionid)
...
def userpop(user):
    if request.method == 'GET':
        ...
        if institutionid is not None:
            ...
            issm_log.logger.info(
                f"Users for particular user  not fetched {user}. Institution id is missing ")
    ...
    if request.method == 'PUT':
        ...
        role = request.form.get('role')
        issm_log.logger.info(
            f"Update for user with {user}, Fullname :{fullname}. email:{email} and role :{role}")
    ...
    if request.method == 'POST':
       ...
       # print('username is ',username)
        ...
        if sign == 'test':
            ...
            add_signature1(pdf_filename, user+'.png', output_file,
                           int(length), int(width), int(xco), int(yco))
            response = make_response(
                send_file(output_file, as_attachment=True))
